...
def _find_pdf_engine() -> str | None:
    ...
    Priority: docx2pdf (Win/Mac) -> libreoffice -> pandoc -> weasyprint -> fpdf2 (fallback)
    """
    ...
    try:
        ...
        pass
    # Pure-Python fallback (text-only, always works)
    try:
...
def _pdf_via_fpdf2(input_path: str | Path, output_path: str | Path) -> ExportResult:
    """Convert DOCX to PDF using fpdf2 (pure-Python, no external dependencies).

    Extracts text from the DOCX and creates a basic PDF with fpdf2.
    Falls back gracefully when the DOCX text cannot be extracted.
    """
    inp = Path(input_path)
    ...
    try:
        ...
        try:
            ...
            pdf.set_font("Helvetica", size=11)
            pdf.multi_cell(0, 6, f"Generated by PiMD from: {inp.name}")
        ...
        if out.exists():
            return ExportResult(
                ...
                format=ExportFormat.PDF,
                success=True,
                warnings=["PDF generated via fpdf2 (text-only, no images/tables)"],
            )
        return ExportResult(
            output_path=out,
            format=ExportFormat.PDF,
            success=False,
            error="fpdf2 failed to create PDF",
        )
    except ImportError:
        return ExportResult(
            output_path=out,
...
def convert_to_pdf(
    ...
    Args:
        ...
        output_path: Explicit PDF output path. If given, the result is copied/renamed here.
    ...
    result: ExportResult
    if engine == "docx2pdf":
        result = _pdf_via_docx2pdf(path)
    elif engine == "libreoffice":
        result = _pdf_via_libreoffice(path, out)
    elif engine == "pandoc":
        result = _pdf_via_pandoc(path, out)
    elif engine == "weasyprint":
        result = _pdf_via_weasyprint(path, out)
    elif engine == "fpdf2":
        pdf_out = Path(output_path) if output_path else (out / f"{path.stem}.pdf")
        return _pdf_via_fpdf2(path, pdf_out)
    else:
        return ExportResult(
            output_path=out / f"{path.stem}.pdf",
            format=ExportFormat.PDF,
    ...
    if result.success and output_path and result.output_path:
        ...
        if result_path != target_path:
            ...
            if result_path.is_file():
                ...
                result.error = "PDF was not created by engine"

    # If primary engine failed and fpdf2 is available, try pure-Python fallback
    if not result.success and source_format == "docx":
        pdf_out = Path(output_path) if output_path else (out / f"{path.stem}.pdf")
        fallback = _pdf_via_fpdf2(path, pdf_out)
        if fallback.success:
            return fallback
...
def doctor() -> list[dict[str, Any]]:
    ...
    for name in ("docx2pdf", "weasyprint", "fpdf"):
        try:
            __import__(name)
